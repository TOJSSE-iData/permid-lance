...
company_save_to = 'tr.test_company_{}'
person_save_to = 'tr.test_person_{}'
...
exchange_map = sc.textFile(data_dir.format('tr_sp_exchange_map')) \
    ...
    .toDF(['origin', 'new'])
exchange = spark.read.table('tr.quote') \
...
raw_name = spark.read.table('tr.organization') \
...
address = spark.read.table('tr.organization') \
...
country = spark.read.table('tr.organization') \
...
has_country = spark.read.table('tr.organization') \
    .filter(F.col('pred') == 'isDomiciledIn') \
    .select('sub') \
    .dropDuplicates()
has_addr = spark.read.table('tr.organization') \
    .filter(F.col('pred') == 'HeadquartersAddress') \
...
url = spark.read.table('tr.organization') \
